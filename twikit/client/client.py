...
class Client:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Set once transaction-id generation is found to be broken (e.g. X changed
        # its web client) so we degrade to sending requests without the header
        # instead of re-attempting — and crashing — on every single request.
        self._client_transaction_disabled = False
    ...
    async def request(
        ...
        **kwargs
        ...
        if (
            ...
            and not self._client_transaction_disabled
        ):
            ...
            try:
                ...
                # X changed its web client and we can no longer derive the
                # x-client-transaction-id. Rather than making the whole library
                # unusable (issues #408/#409/#296), disable the header and warn
                # once; many endpoints still work without it.
                self._client_transaction_disabled = True
                warnings.warn(
                    'Failed to generate x-client-transaction-id '
                    f'({e}). Continuing without it; some requests may be '
                    'rejected by X. Update twikit if this persists.'
                )
            finally:
                self.set_cookies(cookies_backup, clear_cookies=True)

        if self.client_transaction.key is not None:
    ...
    async def _resolve_login_code(
        ...
        """
        Obtain a login challenge code (2FA / email confirmation).

        When ``callback`` is provided it is used (awaited if it is a coroutine
        function), allowing the code to be sourced programmatically from a
        frontend, queue, email inbox, etc. Otherwise the code is read from stdin,
        preserving the original blocking behaviour for interactive use.
        """
        if callback is not None:
            result = callback(prompt_text)
    ...
    async def login(
        ...
        *,
        ...
        totp_callback: Callable[[str], str] | None = None,
        confirmation_code_callback: Callable[[str], str] | None = None
    ) -> dict:
        """
        ...
        totp_callback : Callable[[str], str], default=None
            Callback invoked when a two-factor (2FA) code is required and no
            ``totp_secret`` was supplied. It receives the challenge prompt text
            and must return the 2FA code. May be a regular or ``async`` function.
            When omitted, the code is read from stdin via ``input()`` (the legacy
            behaviour, which blocks — unsuitable for servers/multi-account use).
        confirmation_code_callback : Callable[[str], str], default=None
            Callback invoked when X asks for an email/phone confirmation code
            (the ``LoginAcid`` subtask). It receives the challenge prompt text and
            must return the confirmation code. May be a regular or ``async``
            function. When omitted, the code is read from stdin via ``input()``.
        ...
        Non-blocking login (e.g. fetch the code from your own frontend/queue):

        >>> async def get_code(prompt):
        ...     return await my_frontend.wait_for_code()
        >>> await client.login(
        ...     auth_info_1='example_user',
        ...     password='00000000',
        ...     confirmation_code_callback=get_code,
        ...     totp_callback=get_code
        ... )
        """
        ...
        if flow.task_id == 'LoginAcid':
            prompt_text = find_dict(flow.response, 'secondary_text', find_one=True)[0]['text']
            confirmation_code = await self._resolve_login_code(
                confirmation_code_callback, prompt_text)
        ...
        if flow.task_id == 'LoginTwoFactorAuthChallenge':
            if totp_secret is not None:
                ...
                prompt_text = find_dict(flow.response, 'secondary_text', find_one=True)[0]['text']
                totp_code = await self._resolve_login_code(totp_callback, prompt_text)
